...
# Fmix
def fmix_creater(data, targets, decay_power=3, alpha=1, max_soft=0., reformulate=False):
    lam, mask = sample_mask(alpha, decay_power, (data.shape[-2], data.shape[-1]), max_soft, reformulate)
    ...
    target1, target2 = targets, shuffled_targets
    return data, target1, target2, lam
